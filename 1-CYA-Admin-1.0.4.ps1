...
function Display-Menu {
 ...
 %%((((((((((((((((((((((((((((( %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
   (((((((((((((((((((((((((((((#%%%%%%%%%%%%%%%%%%%%%%%%%%%#           
    ...
    Write-Host -ForegroundColor Green 'Type 990 to Import certificate to the server'
    Write-Host -ForegroundColor Yellow 'Type 991 to show CyberArk Application logs in past 24 hours'
    ...
    Write-Host -ForegroundColor Yellow 'Type 111 to open padr.ini in notepad'
    Write-Host -ForegroundColor Green 'Type 2 to open tsparm.ini in notepad'
    Write-Host -ForegroundColor Yellow 'Type 3 to tail padr.log (in new window)'
    Write-Host -ForegroundColor Green 'Type 4 to tail italog (in new window)'
    Write-Host -ForegroundColor Yellow 'Type 5 to open \server\conf'
    Write-Host -ForegroundColor Green 'Type 6 to open \server\logs'
...
function Open-PadrIni {
    Write-Host -ForegroundColor Yellow "Opening padr.ini with Notepad"
    Start-Process notepad.exe -ArgumentList 'C:\Program Files (x86)\PrivateArk\PADR\Conf\padr.ini'
}
...
try {
    ...
    if ($confirmation -eq "Y") {
        ...
        if ($newValue -eq 1) {
            ...
            Write-Host "Reboot to enable changes."
        } else {
...
# Function to create a scheduled task to reboot the server if a PSMConnect user is not logged in so the users wont get their sessions disconnected
function Create-PSMRebootTask {
    param (
        [string]$scriptPath = "C:\scripts\psm-reboot-gracefully.ps1",
        [string]$taskName = "\CyberArk\PSM Graceful Reboot"
    )
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Description>reboots PSM server when nobody is connected</Description>
    <URI>$taskName</URI>
  </RegistrationInfo>
  <Triggers>
    <TimeTrigger>
...
function Import-TrustedCertificate {
    Write-Host "To import personal certificate with intermediate follow instructions" -ForegroundColor Green
    $location = Read-Host -Prompt 'Type in path to intermediate cert - will be imported to computer trusted root CAs'
    $location2 = Read-Host -Prompt 'Type in path to personal certificate PFX - will be imported to computer personal cert store'
    $mypwd = Get-Credential -UserName 'Enter personal certificate password below' -Message 'Enter personal certificate password below'
    Set-Location Cert:\localmachine\root\
    Write-Host "---------------------------------" -ForegroundColor Red
    Write-Host "Count of root certs BEFORE import" -ForegroundColor Green
    Write-Host @( Get-ChildItem Cert:\localmachine\root\ ).Count;
    Write-Host "Count of private certs BEFORE import" -ForegroundColor Green
    Write-Host @( Get-ChildItem Cert:\localmachine\My\ ).Count;
    Write-Host "---------------------------------" -ForegroundColor Red
    Start-Sleep 5
    Import-Certificate -Filepath "$location"
    Import-PfxCertificate -FilePath "$location2" -CertStoreLocation Cert:\LocalMachine\My -Password $mypwd.Password
    Write-Host "Find imported certificate below line" -ForegroundColor Green
    Write-Host "------------------------------------" -ForegroundColor Red
    Set-Location Cert:\localmachine\root\
    dir
    ...
    dir
    Write-Host "-----------------------------------------" -ForegroundColor Red
    Write-Host "You shold see imported certificates above, scroll up to see details" -ForegroundColor Green
    Write-Host "---------------------------------" -ForegroundColor Red
    Write-Host "Count of root certs AFTER import" -ForegroundColor Green
    Write-Host @( Get-ChildItem Cert:\localmachine\root\ ).Count;
    Write-Host "Count of private certs AFTER import" -ForegroundColor Green
    Write-Host @( Get-ChildItem Cert:\localmachine\My\ ).Count;
    Write-Host "---------------------------------" -ForegroundColor Red
    }

# Function to tail cyberark application logs
function Tail-CyberArkApplicationLogs {
    Write-Host -ForegroundColor Yellow "Tailing CyberArk Application logs..."

    $last24Hours = (Get-Date).AddHours(-24)
    $logs = Get-EventLog -LogName Application | Where-Object {
        ($_.Source -like "CyberArk*" -or $_.Source -like "Cyber-Ark*") -and $_.EntryType -ne "Information" -and $_.TimeGenerated -ge $last24Hours
    } | Sort-Object TimeGenerated | Select-Object TimeGenerated, Source, EventID, EntryType, Message

    $logCount = $logs.Count
    $logs | Format-Table -AutoSize

    Write-Host -ForegroundColor Green "Displayed $logCount logs from the last 24 hours."
}
